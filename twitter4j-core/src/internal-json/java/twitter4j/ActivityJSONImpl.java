...
class ActivityJSONImpl extends TwitterResponseImpl implements Activity {
	...
	final void init(final JSONObject json) throws TwitterException {
		try {
			action = Action.fromString(ParseUtil.getRawString("action", json));
			maxPosition = ParseUtil.getLong("max_position", json);
			minPosition = ParseUtil.getLong("min_position", json);
			createdAt = ParseUtil.getDate("created_at", json, "EEE MMM dd HH:mm:ss z yyyy");
			sourcesSize = ParseUtil.getInt("sources_size", json);
			targetsSize = ParseUtil.getInt("targets_size", json);
			...
			targetObjectsSize = ParseUtil.getInt("target_objects_size", json);
